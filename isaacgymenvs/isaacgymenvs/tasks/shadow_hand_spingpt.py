...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor, object_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Redefine orientation alignment reward with refined temperature
    alignment_diff = quat_mul(object_rot, quat_conjugate(goal_rot))
    alignment_score = torch.abs(alignment_diff[:, 3])
    temperature_orientation = 1.0
    adjusted_orientation_reward = torch.exp(temperature_orientation * (alignment_score - 1.0))

    # Improved stability reward using distance to goal with adjusted scale
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    temperature_stability = 5.0
    reevaluated_stability_reward = 1.0 / (1.0 + temperature_stability * distance_to_goal)

    # Enhanced angular velocity penalty with new scale
    angular_velocity_score = torch.sum(object_angvel ** 2, dim=-1)
    temperature_angvel = 0.1
    enhanced_angular_velocity_penalty = -torch.exp(-temperature_angvel * angular_velocity_score)

    # New task achievement reward based on alignment and reduced velocity
    temperature_task = 0.5
    task_achievement_reward = torch.exp(temperature_task * (alignment_score - 1.0) + enhanced_angular_velocity_penalty)

    # Total reward calculation
    total_reward = adjusted_orientation_reward + reevaluated_stability_reward + enhanced_angular_velocity_penalty + task_achievement_reward

    # Collect reward components
    reward_components = {
        "adjusted_orientation_reward": adjusted_orientation_reward,
        "reevaluated_stability_reward": reevaluated_stability_reward,
        "enhanced_angular_velocity_penalty": enhanced_angular_velocity_penalty,
        "task_achievement_reward": task_achievement_reward
    }

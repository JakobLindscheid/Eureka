...
class FrankaCabinetGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.franka_grasp_pos, self.drawer_grasp_pos, self.cabinet_dof_pos, self.cabinet_dof_vel, self.franka_lfinger_pos, self.franka_rfinger_pos)
...
def compute_reward(
    ...
    # Reward Component 1: Proximity Reward
    # Encourage the hand to move closer to the drawer handle
    proximity_dist = torch.norm(drawer_grasp_pos - franka_grasp_pos, dim=-1)
    proximity_reward = 1.0 / (1.0 + proximity_dist)
    
    # Reward Component 2: Pull Reward
    # Encourage the drawer to be opened more (increase position)
    target_position_delta = 0.8  # Target position change to open the door
    pull_reward_scale = 10.0
    pull_reward = pull_reward_scale * torch.clamp(cabinet_dof_pos[:, 3] - target_position_delta, min=0.0)

    # Reward Component 3: Velocity Reward
    # Encourage continuous smooth movement of the door
    velocity_scale = 5.0
    velocity_reward = velocity_scale * torch.clamp(cabinet_dof_vel[:, 3], min=0.0)

    # Total Reward
    total_reward = proximity_reward + pull_reward + velocity_reward

    # Apply transformations (optional normalization)
    proximity_temp = 1.0
    pull_temp = 1.0
    velocity_temp = 1.0
    proximity_reward_transformed = torch.exp(proximity_reward / proximity_temp) - 1.0
    pull_reward_transformed = torch.exp(pull_reward / pull_temp) - 1.0
    velocity_reward_transformed = torch.exp(velocity_reward / velocity_temp) - 1.0

    transformed_total_reward = proximity_reward_transformed + pull_reward_transformed + velocity_reward_transformed

    # Return the total reward and the individual components for analysis
    rewards_dict = {
    ...
    return transformed_total_reward, rewards_dict

...
def compute_reward(root_states: torch.Tensor, dt: float, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    distance_to_target = torch.sqrt(to_target[:, 0]**2 + to_target[:, 1]**2 + 1e-5)

    # Reward components
    reward_forward_speed = forward_velocity  # Reward based on forward speed
    sustained_speed = torch.clamp(forward_velocity * dt, min=0.0)   # Emphasize speed over time and clamp it
    
    # Heading direction towards the target
    direction_to_target = to_target / distance_to_target.unsqueeze(-1)  # Normalize direction
    heading_dot_product = torch.sum(torso_velocity * direction_to_target, dim=-1)  # Align the heading 

    # New temperature parameters for transformations
    temperature_forward = 2.0
    temperature_sustained_speed = 3.0
    temperature_heading = 2.0
    ...
    # Total reward calculation with adjusted scaling
    total_reward = (transformed_reward_forward_speed + transformed_sustained_speed + transformed_reward_heading) / 3

...
def compute_reward(root_states: torch.Tensor, dt: float, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    to_target = targets - torso_position
    distance_to_target = torch.sqrt(to_target[:, 0]**2 + to_target[:, 1]**2) + 1e-5  # Adding a small value to prevent division by zero

    # New reward components with better scaling
    reward_forward_speed = forward_velocity  # Continue to use forward speed
    sustained_speed = torch.sum(torso_velocity**2, dim=1)  # Use squared speed for sustained speed
    ...
    # Adjust the parameters for transformations
    temperature_forward = 4.0
    temperature_sustained_speed = 5.0
    temperature_heading = 6.0

...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    speed_reward = torch.clamp(forward_velocity / 2.0, min=0.0, max=1.0)  # Scale down speed reward to enhance impact

    # Improved target reach reward that incentivizes movement towards the target while factoring distance
    distance_to_target = torch.norm(targets - torso_position, p=2, dim=-1)
    target_reach_reward = torch.exp(-distance_to_target / target_reach_temp)  # Closer is better

    # Improved alignment reward to normalize based on direction towards the target
    ...
    # Stronger movement penalty for lack of movement

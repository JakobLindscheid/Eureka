...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Forward velocity component and its magnitude
    forward_velocity = velocity[:, 0]
    
    # Quantifying speed: better to reward based on squared velocity
    speed_reward = torch.clamp(forward_velocity ** 2, min=0.0, max=10.0)  # Using squared value for variation and range
    speed_reward_transformed = torch.exp(speed_reward / speed_temp)

    # Reward alignment: calculate directional cosine for forward velocity with target direction
    direction_to_target = targets - torso_position
    distance_to_target = torch.norm(direction_to_target, p=2, dim=-1)
    direction_to_target_normalized = direction_to_target / (distance_to_target.unsqueeze(-1) + 1e-6)  # Avoid division by zero
    alignment_reward = torch.clamp(torch.sum(velocity * direction_to_target_normalized, dim=-1) / (torch.norm(velocity, p=2, dim=-1) + 1e-6), min=-1.0, max=1.0)

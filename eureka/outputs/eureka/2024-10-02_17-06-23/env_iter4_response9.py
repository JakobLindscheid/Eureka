...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.dof_vel)
...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    movement_penalty_temp = 0.4  # Increase strength for stationary ant
    
    # Extract necessary components
    torso_position = root_states[:, 0:3]
    velocity = root_states[:, 7:10]
    
    # Normalize forward velocity component (reduced scaling for adjustment)
    forward_velocity = velocity[:, 0] / 3.0  # Normalize to a range
    speed_reward = torch.clamp(forward_velocity, min=0.0, max=1.0)

    # Target reach reward based on distance (rewriting to provide effective feedback)
    distance_to_target = torch.norm(targets - torso_position, p=2, dim=-1)
    target_reach_reward = torch.clamp(1.0 - (distance_to_target / 10.0), min=0.0, max=1.0)  # Normalize to a range

    # Improved alignment reward (normalization)
    direction_to_target = (targets - torso_position) / (distance_to_target.unsqueeze(-1) + 1e-6)
    alignment_reward = torch.clamp(torch.sum(velocity * direction_to_target, dim=-1), min=0.0, max=1.0)
    ...
    # Stronger movement penalty for remaining stationary (rewriting to enhance effect)
    movement_penalty = -torch.exp(-torch.norm(velocity, p=2, dim=-1) / movement_penalty_temp)

    # Total reward combines all components
    total_reward = speed_reward_transformed + target_reach_reward_transformed + alignment_reward_transformed + movement_penalty

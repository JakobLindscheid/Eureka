...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.dt)
...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Calculate actual forward progress towards the target distance
    distance_to_target = targets[:, 0] - torso_position[:, 0]  # x-component only
    normalized_forward_progress_reward = torch.clamp(distance_to_target / (10 + 1e-5), min=0.0, max=1.0)

    # Simplified task score calculation by combining speed and forward progress (replacing the high-scoring task score)
    task_score = normalized_speed_reward + normalized_forward_progress_reward
    normalized_task_score = torch.exp(task_score / task_score_temp) - 1  

    # Total reward
    total_reward = normalized_speed_reward + normalized_forward_progress_reward + normalized_task_score

    # Creating the reward components dictionary
    reward_components = {
        'speed_reward': normalized_speed_reward,
        'forward_progress_reward': normalized_forward_progress_reward,

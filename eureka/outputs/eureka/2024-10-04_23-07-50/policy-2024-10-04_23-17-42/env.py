...
class BallBalanceDS(VecTask):
    ...
    def compute_reward(self):
        self.rew_buf[:], self.rew_dict = compute_reward(self.ball_positions, self.ball_linvels, self.dof_positions, self.dof_velocities)
...
def compute_reward(ball_positions: torch.Tensor, ball_linvels: torch.Tensor, dof_positions: torch.Tensor, dof_velocities: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    stability_temp = 0.5  # Adjusted to enhance the sensitivity of the reward
    velocity_temp = 2.0   # Increased to emphasize slower, more controlled movements
    position_temp = 0.5    # Adjusted to enhance sensitivity
    ...
    # Stronger penalty for ball's linear velocity; penalize if it moves too fast (higher speed is less desirable)
    ...
    # Total reward combines all components, modified with more dynamic ranges

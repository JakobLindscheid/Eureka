...
def compute_reward(root_states: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Assuming root_states contains the velocities in proper slices.
    ...
    running_reward = torch.exp(forward_velocity * running_temp) - 1  # Adjusted for better scaling

    # Updated action penalty focusing on action irregularity
    action_magnitude = torch.norm(actions, p=2, dim=-1)  # Use L2 norm
    action_penalty = -torch.exp(action_magnitude * action_temp)  # Exponential penalty for large actions

...
def compute_reward(root_states: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    speed = torch.norm(velocity, p=2, dim=-1)  # Overall speed magnitude
    forward_velocity = velocity[:, 0]  # Forward (x) component of velocity

    # Reward components
    speed_reward = speed * 2.0  # Increased sensitivity to overall speed
    direction_bonus = torch.clamp(forward_velocity, min=0.5)  # Reward maintaining minimum forward speed
    action_penalty = -torch.norm(actions, p=2, dim=-1) ** 3  # Cubed penalty for action usage
